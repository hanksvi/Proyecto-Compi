...
Token* Scanner::nextToken() {
    ...
    if(LineStart){
        ...
        if(count > lasIdent){
            ...
            return new Token(Token::INDENT);
        }
        else if(count < lasIdent){
            while(!indentStack.empty() && indentStack.back() > count){
    ...
    // ID
        ...
        else if (lexema=="proc") return new Token(Token::PROC, input, first, current - first);
